...
public class KeyCounts
{
	...
	public static void outputKeyCounts(String filename)
	{
		try
		{
			...
			while(keySetIterator.hasNext())
			{
				...
				System.out.println("The total for " + key + " is " + counts.get(key) + ".");
			}
			System.out.println("-----");
		}
		catch (IOException e)
	...
	public static HashMap<String, Integer> sumKeyCounts(String filename) throws IOException
	{
		HashMap<String, Integer> counts = new HashMap<String, Integer>();
		
		// Load the file from the local directory
		BufferedReader br = new BufferedReader(new InputStreamReader(KeyCounts.class.getClassLoader().getResourceAsStream(filename)));
	    try
	    {
	        String line = br.readLine();

	        // For each line, either add to the existing entry or create a new one
	        while (line != null)
	        {
